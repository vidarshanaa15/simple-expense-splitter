    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-6 flex flex-col sm:flex-row gap-4">
        <button
          ...
          class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-lg shadow-md transition duration-200 flex items-center justify-center gap-2"
        >
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 relative">
          <div class="relative">
            ...
            <input
              v-model="searchQuery"
              type="text"
              class="block w-full pl-10 pr-3 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
          ...
          <p v-if="searchQuery && filteredGroups.length > 0" class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Found {{ filteredGroups.length }} group{{ filteredGroups.length !== 1 ? 's' : '' }}
          </p>
          <p v-if="searchQuery && filteredGroups.length === 0" class="mt-1 text-sm text-amber-600 dark:text-amber-400">
      ...
      <div v-if="recentGroups.length > 0 && !searchQuery" class="mb-8">
      ...
      <div>
        ...
        <!-- No Search Results -->
        <div v-else-if="searchQuery && filteredGroups.length === 0" class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow">
          <svg class="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">No groups found</h3>
          <p class="mt-2 text-gray-600 dark:text-gray-400">Try searching with a different term</p>
          <button
            @click="clearSearch"
            class="mt-4 text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium"
          >
            Clear search
          </button>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-3" v-html="highlightText(group.name)"></h3>
            ...
            <div class="flex flex-wrap gap-2">
              <span
                ...
                :class="matchesSearch(member) ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 ring-2 ring-yellow-400' : 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200'"
                v-html="highlightText(member)"
              >
              </span>
            </div>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6">
        ...
        <form @submit.prevent="createGroup">
          ...
          <div class="flex gap-3">
            ...
            <button
              type="submit"
              :disabled="creating || validationErrors.groupName || validationErrors.members"
...
export default {
  ...
  methods: {
    ...
    highlightText(text) {
      ...
      return text.replace(regex, '<mark class="bg-yellow-200 dark:bg-yellow-600 text-gray-900 dark:text-white px-1 rounded">$1</mark>')
    },
